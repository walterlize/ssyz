...
<html xmlns="http://www.w3.org/1999/xhtml">
    ...
    <body>    
        ...
        <table cellpadding="0" cellspacing="1" class="tablist2">
            ...
            <tr>
                <td class="td1" style="width: 123px">月报经费支出情况文字说明</td>
                <td class="td1" style="width: 123px">文件选择<?php if (isset($isuploaded)) echo($isuploaded); ?>
                    <input type="hidden" name="moneyId" id="moneyId" value="<?= $monthMoney->monthMoneyId ?>" />
                    <input type="hidden" name="uploadId" id="uploadId" value="<?= $uploadId ?>" />
                    <input type="hidden" name="type" id="type" value="<?= $type ?>" />
                    <input type="hidden" name="subjectId" id="subjectId" value="<?= $subjectId ?>" />
                </td>

                <td class="td2" style="<?= $show3 ?>">          	
                    <input type="file" name="userfile" size="40" />
                    <input type="submit" value="上 传" class="input"/>
                    <input  style="<?=$show4?>" type="button" name="btnReturn" value="删 除" onclick="javascript:if(confirm('您确定要删除?'))window.location.href='<?= base_url() ?>index.php/manager/upload/delete/<?=$uploadId?>/<?= $monthMoney->subjectId ?>';" id="btnReturn" class="input" />
            </tr>
            <tr>
