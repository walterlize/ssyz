...
function GetValidChar(validEnum)
{
    ...
    switch (validEnum)
    {
        ...
        case "Float":
            ...
            break;
        case "Double":
            validChar = "^[1-9]d*.d*|0.d*[1-9]d*$";
       // case "Double1":
         //   validChar = "^[0-9]d*.d*|0.d*[1-9]d*$";
         case "Double1":
          validChar = "^[0-9]+(.[0-9]{1,2})?$";
            break;
        case "Int1":
...
//验证用户名是否被占用
function check_user_name(element1, element2, url) {
    var flag = true;
    var username = $("#" + element1).val();
    if (username == 0 || username == '' || username == null)
        return true;
    $.post(url, {old_username: username}, function (result) {
        if (result != '' && result != 0 && result != null) {
            $("#" + element1).val("");
            $("#" + element2).empty();
            $("#" + element2).append(result);
...
function check_next_password(element1, element2, element3) {
    if (valid_password(element2, element3) == false)
        return false;
...
function check_email(element1, element2, form, url) {
    var flag = true;
    if (check_password(element1, element2, url) == false)
        flag = false;
    if (check(form) == false)
        flag = false;
    return flag;
...
function to_admin(url) {
    $.post(url, function (result) {
        if (result != '' && result != 0 && result != null) {
            ...
            alert('未能成功提交，请刷新页面后重新提交！');
        }
    });
}

function check_base(form, element) {
    var flag = true;
    try {
        flag = check(form);
        if (flag) {
            var type = $("#type").val();
            if (type == "应届毕业生") {
                var city = $("#source_place_province").val();
...
// 下一页处理过程
function next_page(element, url) {
    var edit_value = $("#" + element).val();
    if (edit_value == "" || edit_value == 0 || edit_value == null) {

        location.href = url;
    } else {
        alert('请先保存或取消当前编辑的信息！')
    }
}